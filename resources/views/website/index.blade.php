...
<x-layoutWebSite>
 ...
 <section class="hero-section py-16 md:py-24">
    ...
    <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 text-gray-800">
        اكتشف المشاريع المنزلية المميزة في غزة
    </h1>
    <p class="text-gray-600 mb-8 text-lg">
        منصة تجمع أصحاب المشاريع الصغيرة والخدمات المنزلية في مكان واحد
    </p>
    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <!-- المواقع -->
            <div class="flex items-center gap-2">
                <span class="text-gray-600 text-sm">الموقع:</span>
                <select name="location"
                    class="bg-gray-100 border-none rounded py-2 pr-4 pl-8 text-sm appearance-none">
                    <option value="">جميع المواقع</option>
                    @foreach ($locations as $l)
                        <option value="{{ $l->id }}" {{ request('location') == $l->id ? 'selected' : '' }}>
                            {{ $l->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- زر البحث -->
            <button type="submit"
                id="o9" class="text-white px-6 py-2 rounded-button whitespace-nowrap transition mr-auto">
                بحث
            </button>
        </div>
    </div>
                ...
                @foreach ($categories->take(5) as $category)
                    <a href="#" class="flex flex-col items-center group">
                        <div
                          ...
                          <i class="ri-price-tag-3-line ri-xl text-primary"></i>
    ...
    <a href="{{ route('homePage', ['sort' => 'latest']) }}"
       class="px-4 py-2 rounded-full whitespace-nowrap
              {{ request('sort') == 'latest' || !request('sort') ? 'bg-white text-primary shadow-sm' : 'text-gray-600' }}">
        الأحدث
    </a>

    <a href="{{ route('homePage', ['sort' => 'highest-price']) }}"
       class="px-4 py-2 rounded-full whitespace-nowrap
              {{ request('sort') == 'highest-price' ? 'bg-white text-primary shadow-sm' : 'text-gray-600' }}">
        الأعلى سعرًا
    </a>
</div>
    ...
    @foreach ($projects as $project)
        <div class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition flex flex-col">
            ...
            <div class="h-48 overflow-hidden">
                <img src="{{ $project->img_path }}" alt="{{ $project->title }}" class="w-full h-full object-cover object-top">
            </div>
            ...
            <div class="p-4 flex flex-col flex-grow">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <span class="bg-blue-50 text-primary text-xs px-2 py-1 rounded">{{ $project->category->name }}</span>
                </div>
                ...
                <!-- رابط المراسلة -->
                <a href="{{ route('contact') }}" class="text-primary flex items-center text-sm mb-4">
                    <i class="ri-message-2-line ml-1"></i>
                    مراسلة
                </a>

                <!-- زر الشراء / تسجيل الدخول -->
              <div class="mt-auto">
    @auth
        <form action="{{ route('stripe.session', $project->id) }}" method="POST">
            @csrf
            <button type="submit" id="o9" class="btn-pay text-white px-4 py-2 rounded block text-center mt-3 w-full">
                شراء الخدمة
            </button>
        </form>
    @endauth
    ...
    @guest
        <a href="{{ route('login') }}" id="o9" class="btn-pay text-white px-4 py-2 rounded block text-center mt-3 w-full">
            تسجيل الدخول للشراء
        </a>
    @endguest
</div>
   ...
   fetch(button.getAttribute('href'))
        ...
        if (!response.ok) {
            return response.json().then(data => {
                if (data.redirect) {
                    ...
                    throw new Error('فشل في تنفيذ الطلب.');
                }
            });
        }
    ...
    .then(data => {
        if (data.session_id) {
            ...
            throw new Error("لا يوجد session_id في الرد.");
        }
    })
    .catch(error => {
        console.error('تفاصيل الخطأ:', error);
        alert('حدث خطأ أثناء تنفيذ الدفع! تأكد من تسجيل الدخول وحاول مرة أخرى.');
    });
